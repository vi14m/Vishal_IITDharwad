...
class ExtractionEngine:
    ...
    def _extract_with_groq(self, text: str) -> Tuple[List[PageWiseLineItem], TokenUsage]:
        """
        Extract using Groq API (fast and free)
        """
        print("Step 2: Extracting structured data with Groq...")
        from src.groq_extractor import GroqExtractor
        
        extractor = GroqExtractor()
        return extractor.extract_bill_items(text)
    
    def _extract_with_gemini(self, optimized_text: str) -> Tuple[List[PageWiseLineItem], TokenUsage]:
        """
        Extract using Gemini API (fallback option)
        """
        print("Step 2: Extracting structured data with Gemini...")
        prompt = TEXT_EXTRACTION_PROMPT.format(text=optimized_text)
        
        # Try with maximum safety override
        response = self.model.generate_content(
            prompt,
            safety_settings=[
        ...
        if not response.candidates or not response.candidates[0].content.parts:
            ...
            print(f"Warning: Gemini blocked the response (finish_reason: {finish_reason})")
            print("Attempting to extract with simpler prompt...")
            
            # Try again with a simpler, more direct prompt
            simple_prompt = f"""Extract line items from this bill text and return as JSON.

Text:
{optimized_text[:10000]}  

Return JSON format:
{{"pagewise_line_items": [{{"page_no": "1", "page_type": "Bill Detail", "bill_items": [{{"item_name": "", "item_amount": 0.0, "item_rate": 0.0, "item_quantity": 1.0}}]}}]}}"""
        ...
        try:
            ...
            if response.candidates and response.candidates[0].content.parts:
                ...
                print("Gemini blocked response completely. Returning empty extraction.")
                return [PageWiseLineItem(
                    page_no="1",
                    page_type="Bill Detail",
                    bill_items=[]
